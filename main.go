...
func callHostWithResponse(method string, payload []byte) ([]byte, error) {
	...
	callCode := C.call_host_api(cMethod, req, C.size_t(len(payload)), &response)
	var result []byte
...
func callHostLog(level, message string) {
	payload, _ := json.Marshal(map[string]string{"level": level, "message": message})
	callHost("host.log", payload)
}
